...
public class SimulationCompletedTest {
	private SimulationCompletedServlet simulationCompletedServlet;
	...
	public void setup() throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
		...
		try (MockedStatic<Persistence> mockedPersistence = Mockito.mockStatic(Persistence.class)) {
			simulationCompletedServlet =new SimulationCompletedServlet();
	    }
		
		Field f = simulationCompletedServlet.getClass().getDeclaredField("simulationSendOrCompleteService");
		f.setAccessible(true);
		f.set(simulationCompletedServlet, simulationSendOrCompleteService);
	}
	...
	public void shouldSendRedirectToHome_whenAllOk() throws Exception {
		...
		doReturn(user).when(simulationSendOrCompleteService).completeSimulation(user, idMember, idSimulation);
		doReturn(session).when(req).getSession();
		doReturn(path).when(req).getContextPath();
		
		//test
		simulationCompletedServlet.doPost(req, resp);
	...
	public void shouldDispatcherToError_whenGenericExceptionReceived() throws Exception {
		...
		doReturn(idMember).when(req).getParameter("memberSelect");
		doReturn(idSimulation).when(req).getParameter("simulationId");
		doThrow(runtimeException).when(simulationSendOrCompleteService).completeSimulation(user, idMember, idSimulation);
		...
		//results
		verify(simulationSendOrCompleteService).completeSimulation(user, idMember, idSimulation);
